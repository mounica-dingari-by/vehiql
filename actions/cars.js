...
export async function processCarImageWithAI(file) {
  try {
    ...
    const prompt = `
      ...
      10. Car type (New, Used, Certified Pre-Owned , etc.)
...
export async function getCars(search = "") {
  try {
    ...
    // Fetch cars from the database with optional search
...
export const deleteCar = async (carId) => {
  try {
    ...
    try {
      ...
      console.error("Error with storage operations:", storageErr);
    }
     // Revalidate the cars list page
    revalidatePath("/admin/cars");
    ...
    console.error("Error deleting car by id:", carId);
    return {
      success: false,
      error: error.message,
    };
